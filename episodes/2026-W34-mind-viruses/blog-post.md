...
**The weather.** The small local model infected five agents on Tuesday and two on Wednesday. Same code, same sermon, same ring. The difference between those two days is one persuasive conversation landing or dying. Before publishing, I reran my own headline number and watched it fall by more than half. The escape from patient zero happened every single time. The size of the outbreak is luck.
...
Then read `runs/*/transcripts/`. Do not skip that. The transcripts are where the vaccine breach and the goldfish fossil live. Keep payloads benign, like mine: you get all of the science with geese and none of the harm. The paper is [here](https://arxiv.org/abs/2608.10218).
